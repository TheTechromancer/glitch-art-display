        ...
        for file in files:
            ...
            # if image is an unsupported image type
                ...
                try:
                    ...
                    sp.run(['magick', str(filename), str(new_filename)], check=True)
                    yield new_filename
        ...
        for image in found_images:
            try:
                ...
                if not png_filename.exists():
                    pool.submit(sp.run, ['magick', str(image), str(png_filename)], check=True)
    ...
    if is_cached(png_filename):
        ...
        while 1:
            try:
                ...
                try:
                    sp.run(['magick', str(jpeg_filename), str(png_filename)], check=True)
                    break
                except Exception:
                    continue
